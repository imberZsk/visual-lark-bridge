...
class NewsConfig:
    ...
    def runnable(self) -> bool:
        """配置启用且目标、时刻和来源齐全时返回真。"""
        return bool(self.enabled and self.chat_id and self.times and self.sources)
...
def parse_news_config(payload: object) -> NewsConfig:
    ...
    times = tuple(
        sorted(
            {
                value.strip()
                for value in raw_times if isinstance(value, str)
    ...
    if isinstance(raw_sources, list):
        for index, item in enumerate(raw_sources, start=1):
            ...
            parsed_sources.append(NewsSource(source_name or f"信息源 {index}", source_url))
    # max_items 存储收敛到允许区间内的单次条目上限。
    raw_max_items = source.get("max_items")
    max_items = raw_max_items if isinstance(raw_max_items, int) else DEFAULT_NEWS_MAX_ITEMS
